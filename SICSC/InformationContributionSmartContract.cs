...
{
    /// <summary>
    /// Information contribution smart contract.
    ...
    public class InformationContributionSmartContract : SmartContract
    {
        ...
        {
            // TODO: check args array length
            // TODO use Rewards SC instead Token SC for transfer token 
            if (operation == Operations.InformationContributionAssign)
            {
                return Assign((byte[]) args[0], (byte[]) args[1], (BigInteger) args[2]);
            }

            if (operation == Operations.InformationContributionReward)
            {
                return Rewards((byte[]) args[0], (byte[]) args[1], (BigInteger) args[2]);
            }
        ...
        private static int SwapRate() => Configuration.InformationContributionSmartContractSwapRate;
